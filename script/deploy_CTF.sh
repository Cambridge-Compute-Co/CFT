...
if [ "$CHAIN_NAME" == "sepolia" ]; then
  ...
  forge script --chain sepolia script/DeployCFTToken.s.sol:DeployCFTToken --rpc-url $SEPOLIA_RPC_URL --broadcast --verify -vvvv
  exit 0
fi

if [ "$CHAIN_NAME" == "holesky" ]; then
  # deploy CFT token on holesky chain
  forge script --chain holesky script/DeployCFTToken.s.sol:DeployCFTToken --rpc-url $HOLESKY_RPC_URL --broadcast --verify -vvvv
  exit 0
fi